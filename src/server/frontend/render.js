...
import configureStore from '../../common/_configs/configureStore';
import createInitialState from './createInitialState';
import createRoutes from '../../browser/createRoutes';
import serialize from 'serialize-javascript';
import { Provider } from 'react-redux';
import { createMemoryHistory, match, RouterContext } from 'react-router';
import { resolvingServerFetch } from '../../common/_tools/serverFetch';
import { routerMiddleware, syncHistoryWithStore } from 'react-router-redux';
import { toJSON } from '../../common/_tools/transit';
...
export default function render(req, res, next) {
  ...
  match({ history, routes, location }, async (error, redirectLocation, renderProps) => {
    ...
    try {
      if (!process.env.IS_SERVERLESS) {
        await resolvingServerFetch(() => renderApp(store, renderProps));
      }
